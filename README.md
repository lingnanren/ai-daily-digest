...
仓库已包含 GitHub Actions workflow：`.github/workflows/daily-digest.yml`。它会在每天北京时间 08:00（UTC 00:00）运行，也支持在 Actions 页面手动触发。
...
请在你的 fork 仓库中进入 **Settings → Secrets and variables → Actions → New repository secret**，添加以下 Secrets：

| Secret | 说明 |
|--------|------|
| `GEMINI_API_KEY` | Gemini API Key |
| `SMTP_HOST` | 发件邮箱 SMTP 服务器，例如 QQ 邮箱为 `smtp.qq.com` |
| `SMTP_PORT` | SMTP 端口，QQ 邮箱可用 `465` 或 `587` |
| `SMTP_SECURE` | 是否使用直连 TLS，端口 `465` 用 `true`，端口 `587` 用 `false` 或留空自动判断 |
| `SMTP_STARTTLS` | 是否使用 STARTTLS，端口 `587` 用 `true` 或留空自动判断 |
| `SMTP_USER` | 发件邮箱账号，例如 `your@qq.com` |
| `SMTP_PASS` | SMTP 授权码/应用专用密码，不是邮箱登录密码 |
| `MAIL_FROM` / `SMTP_FROM` | 发件人地址，通常与 `SMTP_USER` 相同 |

可选兜底配置：`OPENAI_API_KEY`、`OPENAI_API_BASE`、`OPENAI_MODEL`。
